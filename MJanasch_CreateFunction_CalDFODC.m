...
% Markus Janasch, Ph.D. Student, KTH, edited by Luise Zeckey
% Created: 2018-02-22, last modified: 2021-05-16
...
function MJanasch_CreateFunction_CalDFODC(InputDataStructure)
    ...
    for ParaNum = 1:length(ParID)
        ...
        if (strncmp(ParID(ParaNum),'V_max',5)) || (strncmp(ParID(ParaNum),'K_PPool',7)) || (strncmp(ParID(ParaNum),'K_FUMPool',9)) || (strncmp(ParID(ParaNum),'K_COAPool',9))
    ...
    for RxnNum = 1:length(N.reaction)
        fprintf(fileID,'\t%6s%1.0f%1s\t%1s%1s%1s\n','pippo(',RxnNum,',1)','= ',N.reaction(RxnNum).kineticLaw.math,';');
    end    
    ...
    for i = 1:length(S)
        if ~(N.species(i).boundaryCondition)
            ...
            if isitthere ~= 0
                ...
                formula = N.reaction(j).kineticLaw.math;
                DER{i,j} = diff(str2sym(formula),Ssym(i));
            else
                DER{i,j}='0';
...
function [ParID_New,ParVal_New] = ExtractParameters(N)
    ...
    for a = 1:length(N.reaction)
        for b=1:length(N.reaction(a).kineticLaw.localParameter)
            ParID_New{Running_Variable}     = N.reaction(a).kineticLaw.localParameter(b).name;
            ParVal_New(Running_Variable)    = N.reaction(a).kineticLaw.localParameter(b).value;
