...
function [DataOut, N] = MJanasch_Parameter_Sampling_II(iterations,InputDataStructure,MetConcDataIn,MetNames,seed)
%tic
% INPUT:
%
...
% MetConcDataIn = MetConcData_RAW.data(NrOfMetDataSet,:);
% MetNames = MetConcData_RAW.textdata;

for h = 1:length(N.species)
    if ~strncmp(N.species(h).name,'BioM',4)
        MetIndex=find(ismember(MetNames,N.species(h).name));
...
for c = 1:iterations                    % for every interation
    ...
    [dfodc,Parameters(c,:)] = MJanasch_CalDFODC(N,Parameters(c,:),vmax_K_indeces,Fluxes,SFull);
...
% for i = 1:length(S)
%     N.species(i).initialConcentration = 1;
% end
...
[ParID,ParVal] = ExtractParameters(N);   % Get all reaction parameters
...
for p = 1:length(ParID)                 % for every parameter
    ...
    if ((~strcmp(par_aux2,'eq')) && (~strcmp(par_aux2,'max')) && (~strcmp(par_aux2,'PPool')) && (~strcmp(par_aux2,'FUMPool')) && (~strcmp(par_aux2,'COAPool'))) 
